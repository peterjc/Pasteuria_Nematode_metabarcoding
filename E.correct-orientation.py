# Now that we've sorted the amplicons into primer sets we need to flip the amplicons which assemble in the reverse orientation back into the expected orientation #
...
outfile3 = open("seb-amplicons-orientation-corrected.fastq", "w")

# For each fastq record in the reverse orientation output from the previous step ...#
for seq_record in SeqIO.parse("Pas-amplicons-containing-FandR-reverse-orientation.fastq", "fastq"):
	# ... flip it. Keeping the name and appending (RC) so that later on we can see it was flipped. #
...
for seq_record in SeqIO.parse("Seb-amplicons-containing-fandr-reverse-orientation.fastq", "fastq"):
	rcseq = seq_record.reverse_complement(id=True, description=seq_record.description + " (RC)")
